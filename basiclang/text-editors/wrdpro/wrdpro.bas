...
1360 IF F8=>2 THEN F=F-1:FOR I=F8 TO F:AT$(I)=AT$(I+1>1NEXT
1370 GOTO 1170
1380 IFFB<FTHENLINEINPUT"NEW LINErr • ;A$1F=Ft1 IFORI=FTOF8t1STEP-1ELSE111-a
1390 AT$(I>=AT$CI-1>1NEXTIAT$CF8+1>=ASIGOTO 1170
1400 INPUT"DO YOU WISH TO ADD ANY ADDITIONAL ENTRIEs·;z1s
1410 IF LEFT$CZ1$,1)="Y" THEN GOSUB 840
1420 CLOSE:IF FP$=FZ$ THEN KILLFZ$,DR
1430 NAME" •• XYZ •• • ASFPS, DR: FZ$:::FP$ I GOSUB650: CONSOLEC1 'Pl: CLOSE: GOTO 27<R
1440 AT$(F)=H?a+AT$(1)t•-u+AT$(F)IFOR I=2 TO F
1450 PRINTl1,MID$CSTRS(LN),2)t"' •+AT$(I>ILN=LN+101NEXTtRETURN
1460 IFEOF <;;.~ >THENF=:::O: RETURNELSELINE:CNPUT:fl:2, AS: A~>=MID$ (A$, INSTR( AS,"'•> BC>
1470 IF LEFT$CA$,1)="?" THEN Fl=INSTR(A$,"-") ELSE 1500
1480 AT$(1)=MID$(A$,2,F1-2>:F=F+11AT$CF>=MID$CA$,F1t1)
1490 RETURN
1500 F=Ft11AT$CF>=A$IGOTO 1460
1510 INPUT"ARE THE CHANGES CORRECTs;z1s
1520 IFLEFT$<Z1$v1)=•N"THENPRINTuABORTING JOB' NO FILES CHANGED"
1530 IFLEFT$(Z1$,1)=•N•THENCLOSEIGOT0270
1540 GOTO 1400
1550 REM SET-UP FOR EDITING USING BASIC
1560 PRINT"EDITOR ROUTINE FOR TEXT FILES"
1570 PRINT"THIS SECTION WILL SET UP THE FILE FOR EDITING USING THE a
1580 PRINT"EDITOR WHICH IS BUILT INTO BASic.•tPRINTtPRINT
1590 Z=llINPUT"FILE NAME TO EDITU~FZS:GOSUB3370:IFZ=OTHEN1590
1600 Z=l:INPUT"DRIVE NUMBER";DRIGOSUB34001IFZ=O THEN 1600
1610 INPUT·Do YOU WISH A LIST OF THE FILE BEFORE EDITINGn;z1s
1620 :CF LEFT$(Z1$,1):::•y• THEN OPEN •r•,1,Fzs,DRELSE 1670
1630 INPUT.HIT (CR> WHEN READY";zzs:CONSOLE c2,p2
1640 IF EOF(l>THEN 167()
1650 LINEINPUTi1,A$1PRINTA$
1660 GOTO :t640
1670 CONSOLE c1,r1:LOAD FZS,DR
1680 REM PRINT COPY OF TEXT
1690 PRINT.THIS SECTION PRINTS A COPY OF THE TEXT·
1700 GOSUB33901GOSUB 1880:GOSUB1850:INPUT"NUMBER OF COPIES";Nc
1710 INPUT"LINE UP PAPER AND HIT <CR) WHEN READv•;zzs:CONSOLE c2,p2
1720 NT=LN:IFNC>OTHENGOSUB18201NC=NC-1:GOTO 1740
1730 CONSOLEC1,P11PRINT"EXITING COPY PRINT-OLJT•:GOT0270
1740 IFLEFT$EZ2$,1)=•N•THEN IF NC<1THEN 1720 ELSECONSOLE C1vP1:GOT01710
1750 IF NT=OTHEN 1720 ELSE IF NT>O THEN PRINTINT=NT-1:GOTO 1750
1760 NC=NC+1SCONSOLE C1,P11PRINT:PRINT
1770 PRINT"YOUR DOCUMENT IS TOO LONG FOR THE FORM SIZE SELECTED+"
1780 INPUT" DO YOU WANT TO CONTINUE PRINTING·;zzs
1790 IF LEFT$<ZZ$,1)= Y THEN GOSUB 18601GOTO 1710
1800 INPUT"DO YOU WANT TO RESTART";zzs
1810 IF LEFT$CZZ$,1)=•yu THEN 1700 ELSE 1730
...
2470 IF LEFT$<ZZ$v1>="Y" THEN CLOSEIGOT02290
...
2980 IFLEFT$<Z1SY1)= 0 N"THEN~RINT"ABORTING JOB' NO FILES CHANGED"IGOT0270
...
3590 IFLEN(FP$)(10RLEN(FP$))8THENPRINT•ILLEGAL FILE NAME•:GOT03580
3600 RETURN
