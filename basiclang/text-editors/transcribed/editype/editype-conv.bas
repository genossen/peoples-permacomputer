...
175 IF C$=CHR$(7) AND LEN(A$(K))=0 THEN 120
180 PRINT C$;    
190 IF C$=ASC(10) THEN K=K+1:A$(K)=A$(K)+C$:GOTO 120
200 IF C$=CHR$(7) THEN A$(K)=LEFT$(A$(K),LEN(A$(K))-1):GOTO 120
...
3226     IF J$=CHR$(7) THEN C4$=LEFT$(C4$,LEN(C4$)-1):GOTO 3235
3230     C4$=C$+J$
3235     PRINT J$
3240 NEXT A
...
3281     IF J$=CHR$(7) THEN C5$=LEFT$(C5$,LEN(C5$)-1):GOTO 3290
3285     C5$=C5$+J$
3290     PRINT J$;
3300 NEXT A
...
3350 GOTO 3010
     ...
     REM LINES 3800 - 4710 ADAPTED FROM PRINTER TO SCREEN
...
4080     IF X$=ASC(5) THEN 4200 REM UP
4085     IF X$=CHR$(10) THEN 4660 REM RETURN
4090     P$=P$+X$
...
4662 PRINT P$
4665 IN=(80-LL)/2
4666 DE$=STR$(SL):IF FL=1 THEN GOTO 4667
4667 IF FL=1 THEN PRINT &2,CHR$(17)DE$"."SPC(IN-LEN(DE$)-1)P$SPC(75-LEN(P$)-IN)DE$"."
4668 LC=LC+1:SL=SL+1:P$="":FL=0:P$=T1$:T1$="":GOTO 4680
4670 PRINT &2,CHR$(17)SPC(IN)P$:P$="":P$=T1$:T1$="":LC=LC+1
4680 IF SD$="D" THEN PRINT:LC=LC+1
...
7002     PRINT &2
7003 NEXT M
7004 PRINT &2:PC=PC+1
7005 PRINT &2,CHR$(17)SPC(70)"PAGE "PC
7006 PRINT &2
7007 REM CLOSE 4
7008 LC=3
7009 GOTO 4060
